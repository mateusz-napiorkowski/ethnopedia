...
const renderPage = (
    ...
    mode: "global" | "local" = "local"
    ) => {
        return render(
            <QueryClientProvider client={queryClient}>
                <MemoryRouter initialEntries={[`/collections/${collectionId}/artworks`]}>
                    <Routes>
                        <Route path="/collections/:collectionId/artworks" element={<AdvancedSearch collectionIds={collectionId} mode={mode} />}/>
                    </Routes>  
                </MemoryRouter>
            </QueryClientProvider>
        );
...
describe("AdvancedSearch tests", () => {
    ...
    it("should navigate to the same page with correctly altered query string and component state after search button is clicked - global mode", async () => {
        mockGetAllCategories.mockReturnValue(fetchedCategories)
        const {getByTestId, getByRole, getByLabelText } = renderPage(queryClient, exampleCollectionId, "global")
        await waitFor(() => getByTestId("advancedSearchComponent"))
        const dropdownMenu = getByTestId("dropdown-menu")
        const inputField = getByRole("textbox")
        const addRuleButton = getByRole("button", {
        ...
        expect(mockUseNavigate).toHaveBeenCalledWith(
            `/global-search?Tytuł=tytuł&Tytuł.Podtytuł=podtytuł`,
            {"state":
                {"rules": [
                    {"field": "Tytuł", "id": expect.any(String), "value": "tytuł"},
                    {"field": "Tytuł.Podtytuł", "id": expect.any(String), "value": "podtytuł"}
